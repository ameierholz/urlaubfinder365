-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 00010 v2: Idempotent — löscht alte Policies vor dem Anlegen
-- ══════════════════════════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "own saved_trips" ON saved_trips;
...
DROP POLICY IF EXISTS "own saved_activities" ON saved_activities;
...
DROP POLICY IF EXISTS "own trip_plans" ON trip_plans;
...
CREATE TABLE IF NOT EXISTS travel_documents (
  ...
  label text, passport_number text, expiry_date date, issued_date date, nationality text,
  insurance_provider text, policy_number text, insurance_expiry_date date, emergency_phone text,
  visa_country text, visa_expiry_date date, vaccination_type text, vaccination_date date,
  contact_name text, contact_phone text, contact_email text, contact_relationship text,
  notes text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
ALTER TABLE travel_documents ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "own travel_documents" ON travel_documents;
...
CREATE TABLE IF NOT EXISTS travel_tips (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text NOT NULL, title text NOT NULL, description text NOT NULL,
  category text NOT NULL CHECK (category IN ('geheimtipp','sehenswuerdigkeit','gastronomie','negativ')),
  location_name text NOT NULL, lat numeric(10,7) NOT NULL, lng numeric(10,7) NOT NULL,
  created_at timestamptz DEFAULT now()
);
ALTER TABLE travel_tips ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "travel_tips read" ON travel_tips;
DROP POLICY IF EXISTS "travel_tips insert" ON travel_tips;
DROP POLICY IF EXISTS "travel_tips modify" ON travel_tips;
DROP POLICY IF EXISTS "travel_tips delete" ON travel_tips;
...
DROP POLICY IF EXISTS "own visited_countries" ON visited_countries;
CREATE POLICY "own visited_countries" ON visited_countries
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── Preisverläufe ─────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS price_trends (
  slug text PRIMARY KEY, destination_name text NOT NULL,
  last_updated timestamptz DEFAULT now(),
  current_min_price numeric(10,2) DEFAULT 0, current_deal_count int DEFAULT 0,
  snapshots jsonb DEFAULT '[]', pauschal jsonb, hotel jsonb, ai jsonb
);
ALTER TABLE price_trends ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "price_trends read" ON price_trends;
...
CREATE TABLE IF NOT EXISTS travel_reports (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text NOT NULL, destination text NOT NULL, country text NOT NULL,
  title text NOT NULL, highlights text NOT NULL DEFAULT '', lowlights text NOT NULL DEFAULT '',
  tips text NOT NULL DEFAULT '', price_range text NOT NULL DEFAULT 'mittel',
  rating int NOT NULL DEFAULT 3 CHECK (rating BETWEEN 1 AND 5),
  recommendation boolean NOT NULL DEFAULT true, cover_image_url text,
  visited_at text NOT NULL, is_published boolean NOT NULL DEFAULT false,
  likes_count int NOT NULL DEFAULT 0, liked_by text[] NOT NULL DEFAULT '{}',
  comments_count int NOT NULL DEFAULT 0,
  created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()
);
ALTER TABLE travel_reports ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "travel_reports public read" ON travel_reports;
DROP POLICY IF EXISTS "travel_reports insert" ON travel_reports;
DROP POLICY IF EXISTS "travel_reports update" ON travel_reports;
DROP POLICY IF EXISTS "travel_reports delete" ON travel_reports;
...
CREATE TABLE IF NOT EXISTS travel_report_comments (
  ...
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text NOT NULL, text text NOT NULL,
  created_at timestamptz DEFAULT now()
);
ALTER TABLE travel_report_comments ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "comments read" ON travel_report_comments;
DROP POLICY IF EXISTS "comments insert" ON travel_report_comments;
DROP POLICY IF EXISTS "comments delete" ON travel_report_comments;
CREATE POLICY "comments read"   ON travel_report_comments FOR SELECT USING (true);
CREATE POLICY "comments insert" ON travel_report_comments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "comments delete" ON travel_report_comments FOR DELETE USING (auth.uid() = user_id);
CREATE INDEX IF NOT EXISTS idx_comments_report ON travel_report_comments(report_id, created_at ASC);

-- ── Community Profile ─────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS community_profiles (
  uid uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text NOT NULL DEFAULT '', photo_url text, bio text,
  location text, nationality text, visited_countries text[] DEFAULT '{}',
  followers_count int NOT NULL DEFAULT 0, following_count int NOT NULL DEFAULT 0,
  reports_count int NOT NULL DEFAULT 0, tips_count int NOT NULL DEFAULT 0,
  groups_count int NOT NULL DEFAULT 0, travel_frequency text, traveler_type text,
  languages text[] DEFAULT '{}', travel_interests text[] DEFAULT '{}',
  created_at timestamptz DEFAULT now()
);
ALTER TABLE community_profiles ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "community_profiles read" ON community_profiles;
DROP POLICY IF EXISTS "community_profiles own write" ON community_profiles;
...
CREATE TABLE IF NOT EXISTS travel_groups (
  ...
  creator_name text NOT NULL, name text NOT NULL,
  description text NOT NULL DEFAULT '', destination text NOT NULL DEFAULT '',
  tags text[] DEFAULT '{}', is_public boolean NOT NULL DEFAULT true,
  members_count int NOT NULL DEFAULT 1, member_ids text[] NOT NULL DEFAULT '{}',
  posts_count int NOT NULL DEFAULT 0, created_at timestamptz DEFAULT now()
);
ALTER TABLE travel_groups ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "travel_groups read" ON travel_groups;
DROP POLICY IF EXISTS "travel_groups insert" ON travel_groups;
DROP POLICY IF EXISTS "travel_groups update" ON travel_groups;
DROP POLICY IF EXISTS "travel_groups delete" ON travel_groups;
...
CREATE TABLE IF NOT EXISTS group_posts (
  ...
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  display_name text NOT NULL, text text NOT NULL,
  likes_count int NOT NULL DEFAULT 0, created_at timestamptz DEFAULT now()
);
ALTER TABLE group_posts ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "group_posts read" ON group_posts;
DROP POLICY IF EXISTS "group_posts insert" ON group_posts;
DROP POLICY IF EXISTS "group_posts delete" ON group_posts;
