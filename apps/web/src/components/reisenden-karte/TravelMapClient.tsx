...
const TravelMapLeaflet = dynamic(() => import("./TravelMapLeaflet"), { ssr: false });
...
export default function TravelMapClient() {
  ...
  async function submitTip() {
    ...
    setError(null);
    try {
      const id = await addTravelTip(user.uid, {
      ...
      const newTip: TravelTip = {
        id, userId: user.uid,
        displayName: user.displayName ?? user.email?.split("@")[0] ?? "Anonym",
        ...form, lat: pendingLat, lng: pendingLng, createdAt: new Date().toISOString(),
      };
      setUserTips((prev) => [...prev, newTip]);
      cancelModal();
    } catch {
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 96px)", minHeight: "500px" }}>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[600] flex items-end sm:items-center justify-center p-4 sm:p-0">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full sm:max-w-md mx-auto overflow-hidden">
            {/* Header */}
            <div className="bg-teal-600 px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-teal-100 text-xs mt-0.5">
                  {pendingLat !== null ? `📍 ${pendingLat.toFixed(3)}°, ${pendingLng!.toFixed(3)}°` : "Ort noch nicht gesetzt"}
                </p>
              </div>
