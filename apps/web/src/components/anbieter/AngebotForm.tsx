...
import { Plus, X, Loader2, CheckCircle, Upload, ImageIcon, Star } from "lucide-react";
...
// ── Foto-Upload ───────────────────────────────────────────────────────────────
function FotoUpload({
  hauptfoto, weiterefotos, onHauptfoto, onAddFoto, onRemoveFoto, uploading,
}: {
  hauptfoto: string;
  weiterefotos: string[];
  onHauptfoto: (url: string) => void;
    ...
    <div className="space-y-4">
      ...
      <div>
        <p className="text-xs font-semibold text-gray-600 mb-1.5">
          ...
          <span className="text-gray-400 font-normal ml-1">— wird als Vorschaubild und Hero angezeigt</span>
        </p>
        {hauptfoto ? (
          <div className="relative h-52 rounded-2xl overflow-hidden group">
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              <button type="button" onClick={() => refMain.current?.click()}
                className="bg-white text-gray-800 text-xs font-bold px-4 py-2 rounded-xl">
                Ersetzen
              </button>
              <button type="button" onClick={() => onHauptfoto("")}
                className="bg-red-500 text-white text-xs font-bold px-4 py-2 rounded-xl">
                Entfernen
              </button>
            </div>
          ...
          <div
            onClick={() => refMain.current?.click()}
            className="border-2 border-dashed border-gray-200 rounded-2xl h-52 flex flex-col items-center justify-center cursor-pointer hover:border-[#00838F] hover:bg-[#00838F]/5 transition-all group"
          >
            {uploading ? (
      ...
      <div>
        <p className="text-xs font-semibold text-gray-600 mb-1.5">
          Weitere Fotos
          <span className="text-gray-400 font-normal ml-1">— bis zu 5 zusätzliche Bilder</span>
        </p>
...
interface AngebotData {
  ...
  kategorie?: string; ziel?: string; land?: string; preis?: number; dauer?: string;
  max_teilnehmer?: number; sprachen?: string[]; highlights?: string[];
...
export default function AngebotForm({ anbieter_id, initial }: { anbieter_id: string; initial?: AngebotData }) {
  ...
  const handleAddFoto = (url: string) => {
    setWeiterefotos((prev) => [...prev, url]);
    setUploadingFoto(false);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!titel || !kategorie || !ziel || !preis) { setError("Bitte alle Pflichtfelder ausfüllen."); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <p className="text-xs text-gray-400 mb-5">Gute Fotos sind entscheidend — Angebote mit Fotos werden 3× häufiger gebucht</p>
        <FotoUpload
          hauptfoto={hauptfoto}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Field label="Preis pro Person (€)" required>
            <input type="number" min="0" step="0.01" value={preis} onChange={(e) => setPreis(e.target.value)}
              placeholder="49.00" className={INPUT} />
          </Field>
